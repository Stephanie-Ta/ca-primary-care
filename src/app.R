...
ui <- fillPage(
  ...
  sidebarLayout(
    sidebarPanel(
      selectInput(
        "selected_profession_type",
        "Profession Type",
        choices = c("All", "Family medicine doctors", "Nurse practitioners"),
        selected = "All"
      ),
      selectInput(
        "selected_province_territory",
        "Province/Territory",
        choices = c(
          "Alberta",
          "British Columbia",
...
server <- function(input, output, session) {
  output$lineplot <- renderPlot({
    data |>
      ...
      filter(if (length(input$selected_province_territory) > 0)
        province_territory %in% input$selected_province_territory
        else
          TRUE) |>
      ggplot(aes(x = year, y = count_per_100000, color = province_territory)) +
      geom_line(size = 1) +
      labs(x = "Year", y = "Number per 100,000 Population", color = "Province/Territory") +
      theme_minimal() +
  ...
  output$genderplot <- renderPlot({
    data |>
      ...
      pivot_longer(
        cols = c(percent_female, percent_male),
        names_to = "gender",
  ...
  output$ageplot <- renderPlot({
    data |>
      ...
      group_by(year) |>
      summarise(
        percent_age_under_30 = mean(percent_age_under_30, na.rm = TRUE),
      ...
      scale_fill_manual(
        values = c(
          "percent_age_under_30" = "#72d660",
          "percent_age_30_to_59" = "#339ff2",
          "percent_age_over_60" = "#8b29e6"
        ),
        labels = c(
          "percent_age_under_30" = "Percent Under 30 Years Old",
          "percent_age_30_to_59" = "Percent Between 30 and 59 Years Old",
          "percent_age_over_60" = "Percent Over 60 Years Old"
        )
      ) +
