...
read_save_process_province_territory_data <- function(sheet_number, name) {
  ...
  processed_province_territory_data <- processed_province_territory_data |>
    ...
    mutate(
      province_territory = name,
      count = as.numeric(gsub("[^0-9.]", "", count)),
      count_per_100000 = as.numeric(gsub("[^0-9.]", "", count_per_100000))
    ) |>
    mutate(
      profession_type = case_when(
        profession_type == "Family medicine" ~ "Family medicine doctors",
...
main <- function() {
  ...
  for (i in seq_along(names)) {
    ...
    data = read_save_process_province_territory_data(sheet_number = sheet_number, name =
                                                       names[i])
  ...
  full_data <- full_join(full_data, population_data, by = join_by(year, province_territory))
  ...
  summary_data <- full_data |>
    mutate(
      count_female_per_100000 = count_per_100000 * percent_female / 100,
    ...
    summarize(
      ...
      percent_female = ifelse(
        count_per_100000 == 0,
        0,
        sum(count_female_per_100000, na.rm = TRUE) / count_per_100000 * 100
      ),
      percent_age_under_30 = ifelse(
        count_per_100000 == 0,
        0,
        sum(count_age_under_30_per_100000, na.rm = TRUE) / count_per_100000 * 100
      ),
      percent_age_30_to_59 = ifelse(
        count_per_100000 == 0,
        0,
        sum(count_age_30_to_59_per_100000, na.rm = TRUE) / count_per_100000 * 100
      ),
      percent_age_over_60 = ifelse(
        count_per_100000 == 0,
        0,
        sum(count_age_over_60_per_100000, na.rm = TRUE) / count_per_100000 * 100
      ),
